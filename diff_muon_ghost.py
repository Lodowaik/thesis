...
def main():
  ...
  histos["diff_muon_pt_ghost"] = TH1F("diff_muon_pt_ghost", "diff_muon_pt_ghost; diff_muon_pt_ghost [GeV]; entries", 100, -15, 15)
  histos["diff_muon_eta_ghost"] = TH1F("diff_muon_eta_ghost", "diff_muon_eta_ghost; diff_muon_eta_ghost; entries", 100, -1, 1)
  histos["diff_muon_phi_ghost"] = TH1F("diff_muon_phi_ghost", "diff_muon_phi_ghost; diff_muon_phi_ghost; entries", 100, -1, 1)
    ...
    for imu in range(0,len(e.muon_pt)):
      ...
      #histos["muon_origin"].Fill(e.muon_truthOrigin[imu])
       ...
       # for k in range (0, min(len(e.muon_pt), len(e.truthmuon_pt))):
        ...
        histos["diff_muon_pt_ghost"].Fill(diff_pt/1000.)
        diff_eta=(e.muon_eta[imu])-(e.truthmuon_eta[k])
        histos["diff_muon_eta_ghost"].Fill(diff_eta)
        diff_phi=(e.muon_phi[imu])-(e.truthmuon_phi[k])
        histos["diff_muon_phi_ghost"].Fill(diff_phi)
